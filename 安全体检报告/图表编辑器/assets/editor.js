(() => {
  ...
  const state = { docx: null, docxFile: null, selectedPath: '', selectedMediaItem: null, replacements: new Map(), charts: [], activeChart: null, reportData: null, sourceHtml: '', media: [], mediaMetadata: new Map(), renderVersion: 0, frameLoadVersion: 0, edits: new Map(), previewBlob: null, previewPath: '', previewPending: false };
  ...
  const MEDIA_COMPONENTS = [
    { root: '#slot-risk-overview', charts: ['riskOverviewFrame'], iframe: 'ov4-iframe' },
  ...
  // 这两个 kind 都不需要 ECharts 实例，预览的就绪判定要把它们算作已就绪。
  function hasNoEchartsInstance(chart) { return chart?.kind === 'domText' || chart?.kind === 'attackChain'; }
  ...
  function renderChartGroup(holder, chart) {
    ...
    if (chart.kind === 'attackChain') {
      ...
      const editable = chart.kind !== 'iframe' && chart.kind !== 'domText';
  ...
  function applyIframeEdits(doc, component) {
    const chart = state.charts.find((item) => item.id === component.charts[0]);
    if (!chart) return;
    iframeRows(chart).forEach((row) => {
  ...
  function showIframePreview(html, component, chart) {
    ...
    preview.title = `${chart.title} 实时预览`;
    preview.scrolling = 'yes';
    preview.onload = () => applyIframeEdits(preview.contentDocument, component);
  ...
  async function rasterizeIframeComponent(doc, component, target) {
    ...
    if (typeof window.html2canvas !== 'function') throw new Error('HTML 截图组件未加载。');
    applyIframeEdits(doc, component);
  ...
  function selectMedia(item) {
    ...
    try {
      if (component) activateChart(component.charts[0]);
      else {
        activateChart('');
  ...
  function renderMedia() {
    ...
    state.media.forEach((item) => {
      ...
      const component = componentForPath(item.path);
      const title = component && state.charts.find((c) => c.id === component.charts[0])?.title;
  ...
  function loadHtml(file) { const reader = new FileReader(); reader.onerror = () => setStatus('无法读取 HTML 文件。', 'error'); reader.onload = () => { try { state.sourceHtml = String(reader.result); resetRenderFrame(); const match = state.sourceHtml.match(/window\.SECURITY_REPORT_DATA\s*=\s*([\s\S]*?);<\/script>/); if (!match) throw new Error('HTML 中未找到 SECURITY_REPORT_DATA。请选择由本项目生成的报告 HTML。'); state.reportData = JSON.parse(match[1]); state.charts = chartDefinitions(state.reportData); state.edits.clear(); if (!state.charts.length) throw new Error('该 HTML 中没有当前版本支持的图表数据。'); const select = $('chart-select'); select.innerHTML = state.charts.map((c) => `<option value="${c.id}">${escapeXml(c.title)}</option>`).join(''); select.disabled = false; select.onchange = () => activateChart(select.value); const component = componentForSelection(); if (component) activateChart(component.charts[0]); else selectInitialMappedMedia(); renderMedia(); setStatus(`已加载 ${state.charts.length} 个可编辑图表`, 'ok'); } catch (error) { setStatus(error.message, 'error'); } }; reader.readAsText(file, 'utf-8'); }
